...
def ckz_L12C4_B1_55():
    ...
    noi_dung = (
    f"Cho hàm số $f(x)={latex(f)}$. "
    f"Xét tính đúng-sai của các khẳng định sau:")
    g= random.randint(-3,3)*x+random.randint(1,2)*x**2
    C=random.choice([i for i in range(-5, 6) if i!=0])
    
    kq1_T=f"* $g(x)={latex(F+C)}$ là một nguyên hàm của $f(x)$"
    kq1_F=f"$g(x)={latex(F+g)}$ là một nguyên hàm của $f(x)$"
    
    HDG=thay_dau_congtru(f"Nguyên hàm của $f(x)$ phải có dạng ${latex(F)}+C$.")

    kq1=thay_dau_congtru(random.choice([kq1_T, kq1_F]))

    loigiai_1=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq1==kq1_F:
        loigiai_1=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"

    kq2_T=f"* $\\int f(x)dx={latex(F)}+C$"
    kq2_F=f"$\\int f(x)dx={latex(diff(f,x))}+C$"
    
    HDG=f"$\\int f(x)dx={latex(F)}+C$."
    kq2=random.choice([kq2_T, kq2_F])
    loigiai_2=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq2==kq2_F:
        loigiai_2=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"
    ...
    s_c=sp.symbols("c")
    eq1=Eq(F.subs(x,x_0)+s_c,a)
    solution=solve(eq1,s_c)
    c_0=solution[0]

    kq3_T=f"* $F(x)$ là một nguyên hàm của $f(x)$ thỏa mãn $F({x_0})={a}$ thì $F(x)={latex(F+c_0)}$" 
    kq3_F=f"$F(x)$ là một nguyên hàm của $f(x)$ thỏa mãn $F({x_0})={a}$ thì $F(x)={latex(F+c_0+random.randint(1,4))}$"
    
    HDG=(f"$F(x)={latex(F)}+C$.\n\n"
        f"$F({x_0})={a} \\Rightarrow C={phan_so(c_0)}$.\n\n"
        f"Vậy $F(x)={latex(F+c_0)}$.")
    kq3=random.choice([kq3_T, kq3_F])
    loigiai_3=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq3==kq3_F:
        loigiai_3=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"
    ...
    F_1=F+c_0
    x_1 = random.choice([i for i in range(-5, 6) if i!= x_0])
    y_1=F_1.subs(x,x_1)
    kq4_T=f"* $F(x)$ là một nguyên hàm của $f(x)$ thỏa mãn $F({x_0})={a}$ thì $F({x_1})={phan_so(y_1)}$"
    kq4_F=f"$F(x)$ là một nguyên hàm của $f(x)$ thỏa mãn $F({x_0})={a}$ thì $F({x_1})={phan_so(y_1+random.randint(1,3))}$" 
    
    HDG=f"$F(x)={latex(F+c_0)}$ nên $F({x_1})={phan_so(y_1)}$."
    kq4=random.choice([kq4_T, kq4_F])
    loigiai_4=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq4==kq4_F:
        loigiai_4=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"
...
def ckz_L12C4_B1_56():
    ...
    b = random.choice([i for i in range(-5, 6) if i!=0])
    f=a*sin(x)+b*cos(x)
    ham=thay_sin_cos(latex(f))

    noi_dung = (
    f"Giả sử $F(x)$ là một nguyên hàm của hàm số $f(x)={ham}$.\n\n"
    f" Xét tính đúng-sai của các khẳng định sau:")    
    
    kq1_T=f"* $F'(x)={ham}$" 
    kq1_F=thay_sin_cos(f"$F'(x)={latex(a*cos(x)-b*sin(x))}$")
    ...
    kq2_T=f"* $F(x)={latex(integrate(f,x))}+C$"
    kq2_F=f"$F(x)={latex(diff(f,x))}+C$"
    
    HDG=thay_sin_cos(f"$F(x)={latex(integrate(f,x))}+C$.")
    kq2=thay_sin_cos(random.choice([kq2_T, kq2_F]))
    loigiai_2=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq2==kq2_F:
        loigiai_2=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"

    list_goc=[0,pi/2,pi/3,pi/4, pi/6, pi/2, 2*pi/3,3*pi/4,5*pi/6]

    x_1=random.choice(list_goc)
    y_1=b*sin(x_1)
    c_0=a*cos(x_1)

    kq3_T=f"* Biết $F({latex(x_1)})={latex(y_1)}$. Khi đó $C={latex(c_0)}$" 
    kq3_F=random.choice([
    f"Biết $F({latex(x_1)})={latex(y_1)}$. Khi đó $C={latex(c_0+random.randint(1,3))}$",
    f"Biết $F({latex(x_1)})={latex(y_1)}$. Khi đó $C={latex(c_0-random.randint(1,3))}$", ])
    
    HDG=f"$F({latex(x_1)})={latex(y_1)}\\Rightarrow {b}\\sin {latex(x_1)}+{-a}\\cos {latex(x_1)}+C={latex(y_1)}\\Rightarrow C={latex(c_0)}$."
    ...
    x_2=random.choice([t for t in list_goc if t!=x_1])
    ketqua=integrate(f,(x,x_1,x_2))

    kq4_T=f"* $F({latex(x_2)})-F({latex(x_1)})={latex(ketqua)}$"
    kq4_F=f"$F({latex(x_2)})-F({latex(x_1)})={latex(ketqua+random.randint(1,3))}$" 
    
    F=integrate(f,x)+c_0
    f_2=F.subs(x,x_2)

    HDG=(f"$F({latex(x_2)})={latex(f_2)}$.\n\n"
        f"$F({latex(x_2)})-F({latex(x_1)})={latex(ketqua)}$.")
...
def ckz_L12C4_B1_57():
    ...
    noi_dung = (
    f"Giả sử $F(x)$ là một nguyên hàm của hàm số $f(x)={latex(f)}$.\n\n"
    f"Xét tính đúng-sai của các khẳng định sau:")    
    ...
    kq2_T=f"* $F(x)={latex(F)}+C$"
    kq2_F=random.choice([
        f"$F(x)={latex(F+random.randint(1,2)*x)}+C$",
        f"$F(x)={latex(diff(f,x))}$" ])
    ...
    list_x=[i for i in range(-5, 6) if i!=0]

    x_1 = random.choice(list_x)
    y_1=random.randint(-5,5)
    F_1=F.subs(x,x_1)
    c_0=y_1-F_1

    kq3_T=f"* Biết $F({x_1})={y_1}$. Khi đó $C={phan_so(c_0)}$" 
    kq3_F=f"Biết $F({x_1})={y_1}$. Khi đó $C={phan_so(c_0+random.randint(1,2))}$"
    
    HDG=f"$F({x_1})={y_1} \\Rightarrow {phan_so(F_1)}+C={y_1}\\Rightarrow C={phan_so(c_0)}$."
    kq3=random.choice([kq3_T, kq3_F])
    loigiai_3=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq3==kq3_F:
        loigiai_3=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"
    ...
    kq4_T=f"* $F({x_2})-F({x_1})={phan_so(f_2-y_1)}$"
    kq4_F=f"$F({x_2})-F({x_1})={phan_so(f_2-y_1+random.randint(1,3))}$" 
    
    HDG=(f"$F({x_2})={phan_so(f_2)}$.\n\n"
        f"$F({x_2})-F({x_1})={phan_so(f_2-y_1)}$")
    kq4=random.choice([kq4_T, kq4_F])
    loigiai_4=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq4==kq4_F:
        loigiai_4=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"
...
def ckz_L12C4_B4_36():
    ...
    HDG=(thay_dau_congtru(f"$\\int f'(x){d_x}={phan_so(c/a)}\\ln |{latex(a*x+b)}|+C$." ))

    kq2=random.choice([kq2_T, kq2_F])
    kq2=thay_dau_congtru(kq2)
    loigiai_2=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq2==kq2_F:
        loigiai_2=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"
    ...
    HDG=(f"$f({x_3})={latex(f_3)}$.\n\n"
        f"$f({x_4})={latex(f_4)}$.\n\n"
        f"$f({x_3})+f({x_4})={kq}$.")
    HDG=thay_dau_congtru(thay_log_2_ln(HDG))
...
def ckz_L12C4_B4_52():
    ...
    if chon==1:
        ...
        # Viết I dưới dạng p*pi + q
        p = sp.simplify(sp.expand(I).coeff(sp.pi, 1))
        q = sp.simplify(I - p*sp.pi)
        kq  = p+q


        noi_dung = (
            f"Biết $\\int\\limits_0^{{{latex(pi/3)}}}({a}+{b}\\sin x)\\,dx=p\\,\\pi+q$. "
            f"Tính $p+q$."
        )

        noi_dung_loigiai = (

            f"$\\int\\limits_0^{{{latex(pi/3)}}}({a}+{b}\\sin x)\\,dx"
            f"=\\left[{a}x-{b}\\cos x\\right]{gach(0,latex(pi/3))}$\n\n"
            f"$= {a}\\cdot\\frac{{\\pi}}{{3}}-{b}\\cos {latex(pi/3)} + {b}\\cos 0"
            f"= \\frac{{{a}}}{{3}}\\pi +\\frac{{{b}}}{{2}}.$\n\n"
    ...
    if chon==2:
        ...
        noi_dung = (
            f"Biết $\\int\\limits_0^{{{latex(pi/4)}}}({a}+{b}\\sin x)\\,dx=m\\pi+n{latex(sqrt(2))}+p$. "
            f"Tính $m+n+p$."
        )

        noi_dung_loigiai = (

            f"$\\int\\limits_0^{{{latex(pi/4)}}}({a}+{b}\\sin x)\\,dx"
            f"=\\left[{a}x-{b}\\cos x\\right]{gach(0,latex(pi/4))}$\n\n"     
            f"$=\\dfrac{{{a}\\pi}}{{4}}+\\dfrac{{{-b}{latex(sqrt(2))}}}{{2}}-{b}$\n\n"
...
def ckz_L12C4_B4_58():
    ...
    pa_A= thay_dau_congtru(f"*{kq}")
    pa_B= thay_dau_congtru(f"{kq2}")
    pa_C= thay_dau_congtru(f"{kq3}")
    pa_D= thay_dau_congtru(f"{kq4}")
    #Trộn các phương án
    list_PA =[pa_A, pa_B, pa_C, pa_D]
    random.shuffle(list_PA)
    dap_an=my_module.tra_ve_dap_an(list_PA)
...
#[D12_C4_B5_53]-TF-M2. Vật thay đổi với gia tốc a(t)=mt+n. Xét Đ-S:a(t0), v(t), v(t0), quãng đường
def ckz_L12C4_B5_53():
    d_t=f"\\mathrm{{\\,d}}t"
    m_s=f"\\mathrm{{\\,m}}/\\mathrm{{\\,s}}"
    m_s2=f"\\mathrm{{\\,m}}/\\mathrm{{\\,s}}^2"
    ...
    while True: 
        ...
        if f.subs(t,t_2)<=0:
            continue              
        s_t=float(integrate(f, (t,{t_1},{t_2})))
        if s_t<999:
            break

    if s_t.is_integer():
        s=int(s_t)
        s_f=int(s_t+random.randint(2,3))
        lam_tron=f""
    else:
        s=f"{round_half_up(s_t,0):.0f}".replace(".",",")
        s_f=f"{round_half_up(s_t+random.randint(2,3),0):.0f}".replace(".",",")
        lam_tron=f" (kết quả làm tròn đến hàng đơn vị)"
    ...
    noi_dung = (
    f"Một vật đang chuyển động thẳng với vận tốc ban đầu ${c} ({m_s})$"
    f" thì đột ngột thay đổi chuyển động với gia tốc $a(t)={latex(2*a*t+b)} ({m_s2})$, thời gian ${{t}}$ tính bằng giây, $t=0$ là thời điểm bắt đầu thay đổi chuyển động."
    f" Xét tính đúng-sai của các khẳng định sau:")

    t0=random.randint(2,7)
    a0=2*a*t0+b
    
    kq1_T=f"* Gia tốc của vật tại thời điểm $t={t0}$ là ${a0} {m_s2}$" 
    kq1_F=f"Gia tốc của vật tại thời điểm $t={t0}$ là ${a0+random.randint(1,2)} {m_s2}$"
    
    HDG=f"Gia tốc của vật tại thời điểm $t={t0}$ là $v({t0})={a0} {m_s2}$."
    kq1=random.choice([kq1_T, kq1_F])
    loigiai_1=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq1==kq1_F:
        loigiai_1=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"

    kq2_T=f"* Kể từ khi thay đổi chuyển động vận tốc của vật tại thời điểm ${{t}}$ là $v(t)={latex(f)}$"
    kq2_F=f"Kể từ khi thay đổi chuyển động vận tốc của vật tại thời điểm ${{t}}$ là $v(t)={latex(f+random.randint(1,3))}$"
    
    HDG=(f"$v(t)=\\int ({latex(2*a*t+b)}) {d_t}={latex(a*t**2+b*t)}+C$.\n\n"
        f"$v(0)={c}\\Rightarrow C={c} \\Rightarrow v(t)={latex(f)}$.")
    kq2=random.choice([kq2_T, kq2_F])
    loigiai_2=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq2==kq2_F:
        loigiai_2=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"

    t3=random.randint(2,6)
    v3=f.subs(t,t3)

    kq3_T=f"* Vận tốc của vật tại thời điểm $t={t3}$ là ${v3} {m_s}$" 
    kq3_F=f"Vận tốc của vật tại thời điểm $t={t3}$ là ${v3+random.randint(1,3)} {m_s}$"
    
    HDG=(f"Vận tốc của vật tại thời điểm $t={t3}$ là $v({t3})={v3} {m_s}$.")
    kq3=random.choice([kq3_T, kq3_F])
    loigiai_3=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq3==kq3_F:
        loigiai_3=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"

    kq4_T=f"* Quãng đường vật đi được trong khoảng thời gian từ $t={t_1}$ đến $t={t_2}$ là ${{{s}}}$ m{lam_tron}"
    kq4_F=f"Quãng đường vật đi được trong khoảng thời gian từ $t={t_1}$ đến $t={t_2}$ là ${{{s_f}}}$ m{lam_tron}" 
    
    HDG=(f"Quãng đường đi được là: $  \\int\\limits_{{{t_1}}}^{{{t_2}}} {{\\left({latex(f)}\\right){d_t}}}={s}$ m.")
    kq4=random.choice([kq4_T, kq4_F])
    loigiai_4=f"Khẳng định đã cho là khẳng định đúng.\n\n {HDG}"
    if kq4==kq4_F:
        loigiai_4=f"Khẳng định đã cho là khẳng định sai.\n\n {HDG}"
